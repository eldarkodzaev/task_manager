...
def test_get_new_id(reset_test_data):
...
def test_change_if_task_not_exists(reset_test_data):
    """Случай, когда пользователь пытается редактировать задачу,
    которой нет в базе данных"""
    
    manager = TaskManager(file="test_tasks.json")
    with pytest.raises(ValueError):
        manager.change(task_id=123)  # нет задачи с id 123
        

def test_change_task_fully(reset_test_data):
...
def test_change_task_partial(reset_test_data):
...
def test_mark_as_done(reset_test_data):
...
def test_delete_by_id(reset_test_data):
    """Удаление задачи по id"""
    manager = TaskManager(file="test_tasks.json")
    manager.delete_by_id(task_id=1)
    expected_json = [
        {"id": 2, "status": "не выполнена", "title": "Task 2", "description": "Description for Task 2", "category": "Personal", "due_date": "2023-11-05", "priority": "средний"},
        {"id": 3, "status": "выполнена", "title": "Task 3", "description": "Description for Task 3", "category": "Work", "due_date": "2023-11-08", "priority": "низкий"}
    ]
...
def test_delete_by_category(reset_test_data):
    """Удаление задач по категории"""
    manager = TaskManager(file="test_tasks.json")
    number_of_deleted_tasks = manager.delete_by_category(category="Work")
    expected_json = [
        {"id": 2, "status": "не выполнена", "title": "Task 2", "description": "Description for Task 2", "category": "Personal", "due_date": "2023-11-05", "priority": "средний"}
    ]
    with open("test_tasks.json", "r", encoding="utf-8") as json_file:
